				...
				if( !GameState.HasStarted && DateTime.UtcNow >= GameState.GameEndsAt ) {
					...
					if( playerCount < 6 ) {
						...
						.Select( s => new PlayerDataModel {
							NetId = s,
							IsAlive = true,
							Team = Server.Sessions.FromPlayer( s ).User.Data.PreviousTeam == Team.None ? (playerCount++ * ratio % 1 <= 0 ? Team.Hunter : Team.Prop) : (Server.Sessions.FromPlayer( s ).User.Data.PreviousTeam == Team.Prop ? Team.Hunter : Team.Prop),
						} ).ToList();
					}
		...
		private async Task EndMap( Team winningTeam ) {
			try {
				...
				foreach( var player in GameState.Players ) {
					...
					points += amount;
					playerCount++;
					session.User.Data.PreviousTeam = player.Team;
